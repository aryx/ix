...
# or 7a and 7l) and by ix (tinyasm, tinyld), the executables compared.
...
for f in "$@"; do
  ...
  { $IX/assembler/Main.exe -m $O -o $d/t.$O $f && timeout 10 $IX/linker/Main.exe -m $O ${H:--H7} -E ${E:-_start} -o $d/t.exe $d/t.$O; } 2>$d/t.log || { echo "TINY-FAIL $b: $(head -1 $d/t.log)"; continue; }
