...
# mk (with -i: TinyMk does not pretend, see Build.mli) and tinymk; each
# directory's stdout and exit status compared exactly.
#
#   tree_differential.sh ~/xix        # (a copy of) xix, or principia
#
# One line per directory: "same" or "diff", 9base's and tinymk's exit
...
# unset, because 9base ignores it and TinyMk does not (Mkfile.mli).
# -n runs no recipes, but a mkfile's backquotes and <| still run.

ROOT=${1:?usage: tree_differential.sh dir}
HERE=$(cd "$(dirname "$0")/../.." && pwd)
TINYMK=${TINYMK:-$HERE/_build/default/builder/Main.exe}
...
find -L . -name mkfile -not -path './_build/*' 2>/dev/null | sort | while read -r f; do
  ...
  ( cd "$d" || exit
    ...
    timeout 30 "$TINYMK" -n > "$OUT/$k.tiny" 2> "$OUT/$k.tiny.err"; echo $? > "$OUT/$k.tiny.st" )
