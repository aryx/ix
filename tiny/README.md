...
| `TinyPi.ml` | tiny-pi | `raspberry/` (mini-qemu, its Pi1; QEMU's raspi1ap) | TinyArm's CPU (`TinyLibArm.ml`) in the Pi1: the modes and their banked r13/r14, CPSR and SPSR, the exceptions (svc, undefined, IRQ) and their return, mrs/msr/cps/wfi, the PL011, the system timer and the interrupt controller at their real addresses, time from instructions; a raw image loaded at 0x8000 as the firmware loads kernel.img; a page of kernel (`TinyPi_tests/tick.s`: user mode, system calls, an undefined instruction, five timer interrupts) that runs the same here, under mini-qemu and under QEMU |
...
(`v0/`: a page of assembly and four programs linked with it; `v1/`, to
come, in C, gradually toward xv6's design), and the C runtime they
