 ...
 * - {b The operating system is three calls}: read, write, exit, as
 *   Linux numbers them (r7), so that the ELF written runs on Linux as
...
let syscall (caps : < caps; .. >) m =
  ...
  | 3 ->
      ...
      if n > 0 then (check m r.(1) n; Bytes.blit b 0 m.mem r.(1) n);
      r.(0) <- m32 n
  | 4 ->
      check m r.(1) r.(2);
      ...
      r.(0) <- r.(2)
  | n -> error "unimplemented system call %d" n

(* one step: the word at pc decoded and run *)
let step caps m =
  let pc = m.r.(15) in
  let w = load32 m pc in
  let i = match decode ~pc w with Some i -> i | None -> error "unimplemented instruction %08x at 0x%x" w pc in
  ...
  (match i with
   ...
   | Mem (c, load, byte, rd, rn, off, index) when passed m c ->
       ...
       if load then set rd (if byte then load8 m a else load32 m a)
       else if byte then store8 m a m.r.(rd) else store32 m a m.r.(rd)
   | Block (c, load, rn, wb, mode, regs) when passed m c ->
       ...
       for k = 0 to 15 do
         if regs land (1 lsl k) <> 0 then begin
           if load then loaded := (k, load32 m !a) :: !loaded else store32 m !a m.r.(k);
           a := !a + 4
         end
       done;
   ...
   | Bx (c, link, rm) when passed m c ->
       ...
       next := t
   | Svc (c, _) when passed m c -> syscall caps m
   | _ -> ());
  m.r.(15) <- !next
...
let run caps image labels args =
  let m = { r = Array.make 16 0; n = false; z = false; c = false; v = false; mem = Bytes.make size '\000' } in
  ...
  store32 m sp (List.length args);
  List.iteri (fun i p -> store32 m (sp + 4 + (4 * i)) p) ptrs;
  m.r.(13) <- sp;
  m.r.(15) <- entry labels;
...
let main (caps : < caps; Cap.argv; Cap.open_in; Cap.open_out; .. >) =
  ...
  try
    match args with
    | "-l" :: file :: _ ->
        let _, _, code = assemble ~origin (read file) in
        (* the word decoded back, so that objdump's text checks both
         * directions *)
        List.iter (fun (a, i) ->
    ...
    | "-o" :: out :: file :: _ ->
        let image, labels, _ = assemble ~origin (read file) in
