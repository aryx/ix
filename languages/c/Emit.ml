...
let arm () = (m ()).thechar = '5'
let nreg () = if arm () then 16 else 32
...
(* an offset as the machine's Adr holds it: 32 bits on arm (5c's) *)
let sx v = if arm () then sx32 v else v
...
let rec naddr (n : expr) : A.operand =
  ...
  match n.e with
  | Reg r -> if r >= nreg () then A.FReg (r - nreg ()) else A.Reg r
...
let init () = progs := []; pc := 0; nstring := 0; Buffer.clear sbuf; suppress := 0
...
let obj file : A.obj =
  ...
  let items = Array.map (fun q ->
    ...
    in
    item, 0) ps in
  { A.arch = (if arm () then A.Arm else A.Arm64); file; items }
