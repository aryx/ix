...
let com64 (n : expr) : expr option =
  ...
  | _ when typev (et n) -> (
      ...
      | Assign (Some o, x, y) ->
          ...
          call (fvn "_vasop" Tvlong) [ addr_of x; { (addr_of a) with complex = 0 }; nodconst (Int64.of_int (etconv (et x))); y ]
...
let rec xcom (n : expr) : expr =
  ...
  if addressable n then n
    ...
    match (if (h ()).com64 then com64 n else None) with
    | Some n -> n
    | None -> (
        match n.e with
...
let bcomplex (n : expr) (sides : (stmt * stmt option) option) =
  ...
  | _ -> boolgen (bool64 n) true None; n, false
