 ...
 * unmount, fauth, fversion: devmnt), brk, errstr; the others fail ("not
 * yet", named on the console): the segments', rendezvous and
 * semaphores. *)
...
(* a note that kills (a trap's): "text pid: suicide: msg" on the
 * process's standard error, then exits msg (notify, no handler) *)
val suicide : proc -> string -> unit
