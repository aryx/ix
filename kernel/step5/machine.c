 ...
 */
/* mini-xv6, step 5 (plan_kernel.md): step 4's machine, and the timer:
 * the BCM2835's system timer (compare 3, xv6 arm-pi1's) and the
 * interrupt controller, a user's IRQ handed to OCaml's "irq", a way to
 * wait with IRQs masked (wfi wakes on a pending interrupt anyway). Step
 * 4: the MMU on. Step 3's processes and switch, and: physical memory as OCaml
 * reaches it (by physical address, which fits an int: the kernel's own
 * addresses, KERNBASE and up, never reach OCaml), the user's
 * translation table switched (TTBR0), a user's fault handed to OCaml
 * with the fault's address and status.
 *
...
/* the user program's image (image.s: user.bin, linked at 0) */
extern char user_image[], user_image_end[];
value image_size(value unit) { (void)unit; return Val_int(user_image_end - user_image); }
value image_copy(value pa, value off, value n)
{
  int i;
  for (i = 0; i < Long_val(n); i++) P2V(Long_val(pa))[i] = user_image[Long_val(off) + i];
  return Val_unit;
}
...
value proc_init(value p, value pc, value sp)
{
  ...
  trapframes[i][16] = 0x10 | 0x40;                 /* USR, IRQs on (FIQ masked) */
